...
export default function BinancePay({ isOpen, onClose, walletInfo, info }) {
  ...
  const binanceWallet = walletInfo?.find(
    (w) => w.wallet_provider === "Binance"
  );

  // Binance ID - you can replace this with your actual Binance ID
  const binanceId = binanceWallet?.binance_id;
  ...
  const handleVerifyPayment = async () => {
    ...
    setIsLoading(true);
    const response = await fetch("/api/user/binance/payment-verify", {
      method: "POST",
      body: JSON.stringify({
        binanceWallet,
        info,
    ...
    if (data.verified) {
      ...
      if (data?.redirect_success_url) {
        router.push(data.redirect_success_url);
      }
    } else {
      toast.error(data.message);
      if (data?.redirect_failed_url) {
        router.push(data.redirect_failed_url);
      }
    }
    setIsLoading(false);
    onClose();
